...
# Create plot 
host_since_plot <- ggplot(joined_summary, aes(x = as.numeric(Var1), y = Freq)) + 
  geom_line(col = "red") + 
  xlab("Date Joined") + 
  ylab("# of Hosts Joined ") + 
  ggtitle("# of Hosts Joined over Time")
...
host_since_plot <- ggplot(combined, aes(x = as.numeric(year))) + 
  geom_line(aes(y = Freq), col = "red") + 
  geom_line(aes(y = value / coeff),col = "blue") + 
  scale_y_continuous(
    name = "Number of people joined",
